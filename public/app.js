...
function renderFallbackServiceGlyph(serviceType) {
    ...
    return `<span class="inline-flex h-full w-full items-center justify-center rounded-[7px] ${meta.wrapperTone || ''}"><i class="${escapeAttr(meta.iconClass || 'fa-solid fa-mobile-screen-button')} ${escapeAttr(meta.iconTone || 'text-slate-700')} text-[0.92em]"></i></span>`;
}

function renderServiceBrandImageGlyph(serviceType) {
    const logoUrl = serviceBrandLogoUrls[serviceType];
    if (!logoUrl) return '';
    const meta = getServiceMeta(serviceType);
    return `
...
async function loadAdminData() {
    ...
    try {
        ...
        const totalProfit = successfulOrders.reduce((sum, order) => {
            const providerCost = Number(order.provider_cost_pkr || 0);
            if (order.profit_pkr != null && Number.isFinite(Number(order.profit_pkr))) {
                return sum + Number(order.profit_pkr || 0);
            }
            return sum + (Number(order.price || 0) - providerCost);
        }, 0);
