            ...
            <article class="payment-detail-row ${escapeAttr(status)}">
                <div class="payment-detail-main">
                    ...
                    <div class="mt-4 grid gap-2 sm:grid-cols-3">
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            <div class="text-[11px] uppercase tracking-[0.18em] text-slate-500">Sender Name</div>
                            <div class="mt-2 break-all text-sm font-semibold text-slate-900">${escapeHtml(request.sender_name || 'Not provided')}</div>
                        </div>
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
...
function renderAdminPaymentRequests(paymentRequests, legacyTransactions) {
    ...
    const rows = items.map((item) => {
        ...
        return `
            <tr class="border-b border-slate-200 align-top last:border-b-0">
                ...
                <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-700">${escapeHtml(item.sender_name || (item.entry_kind === 'legacy_transaction' ? 'Legacy' : 'N/A'))}</td>
    ...
    return renderAdminTable(
        ['Source', 'Customer', 'User Email', 'Sender Name', 'Amount', 'Transaction ID', 'Submitted', 'Proof', 'Actions'],
        rows,
        'min-w-[1360px]'
    );
}
...
function renderAdminPaymentHistory(paymentRequests) {
    ...
    const rows = paymentRequests.map((request) => {
        ...
        return `
            <tr class="border-b border-slate-200 align-top last:border-b-0">
                ...
                <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-700">${escapeHtml(request.sender_name || 'N/A')}</td>
    ...
    return renderAdminTable(
        ['Customer', 'User Email', 'Sender Name', 'Amount', 'Transaction ID', 'Submitted', 'Status', 'Actions'],
        rows,
        'min-w-[1280px]'
    );
}
...
function bindStaticEvents() {
    ...
    qs('addFundsForm').addEventListener('submit', async (event) => {
        ...
        const senderNameInput = qs('payment-sender-name');
        ...
        if (!senderName) {
            showPaymentFormError('Sender name is required');
