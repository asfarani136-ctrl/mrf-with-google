...
const AUTO_PAYMENT_SECRET = process.env.AUTO_PAYMENT_SECRET || 'F@is4l#MRF_2026_SECURE_9x!';
...
async function initDB() {
    ...
    await queryRun('ALTER TABLE payment_requests ADD COLUMN IF NOT EXISTS sender_name TEXT');
    await queryRun('ALTER TABLE payment_requests ADD COLUMN IF NOT EXISTS matched_at TIMESTAMPTZ');
    await queryRun('UPDATE payment_requests SET sender_name = LOWER(TRIM(sender_name)) WHERE sender_name IS NOT NULL');
    await queryRun("CREATE INDEX IF NOT EXISTS idx_payment_requests_pending_sender_amount ON payment_requests (LOWER(sender_name), amount) WHERE status = 'pending'");
    await queryRun(`
DO $$
BEGIN
    IF EXISTS (
...
async function createPendingPaymentRequest({ userId, userEmail, amount, transactionId, screenshot, senderName }) {
    return queryRun(
        'INSERT INTO payment_requests (user_id, user_email, amount, transaction_id, screenshot, sender_name, status) VALUES ($1, $2, $3, $4, $5, $6, $7)',
        [userId, userEmail, amount, transactionId || null, screenshot, senderName, 'pending']
    );
}
...
const instagramCountries = [
    ...
    { name: 'Belgium', code: '+32', price: 30, countryId: 82, flag: '🇧🇪' },
    { name: 'Colombia', code: '+57', price: 25, countryId: 33, flag: '🇨🇴' },
    { name: 'South Africa', code: '+27', price: 25, countryId: 31, flag: '🇿🇦' },
    { name: 'Germany', code: '+49', price: 25, countryId: 43, flag: '🇩🇪' },
    { name: 'Kenya', code: '+254', price: 40, countryId: 8, flag: '🇰🇪' },
    { name: 'Mexico', code: '+52', price: 40, countryId: 54, flag: '🇲🇽' },
    { name: 'Philippines', code: '+63', price: 40, countryId: 4, flag: '🇵🇭' },
    { name: 'Brazil', code: '+55', price: 40, countryId: 73, flag: '🇧🇷' },
    { name: 'Jamaica', code: '+1', price: 45, countryId: 103, flag: '🇯🇲' },
    { name: 'India', code: '+91', price: 50, countryId: 22, flag: '🇮🇳' },
    { name: 'Chile', code: '+56', price: 40, countryId: 151, flag: '🇨🇱' },
    { name: 'Nigeria', code: '+234', price: 50, countryId: 19, flag: '🇳🇬' },
    { name: 'Yemen', code: '+967', price: 50, countryId: 30, flag: '�🇪' },
    { name: 'Iraq', code: '+964', price: 50, countryId: 47, flag: '��' },
    { name: 'Peru', code: '+51', price: 40, countryId: 65, flag: '🇵🇪' },
    { name: 'Venezuela', code: '+58', price: 40, countryId: 70, flag: '🇻🇪' },
    { name: 'Uruguay', code: '+598', price: 70, countryId: 156, flag: '🇺🇾' },
    { name: 'Japan', code: '+81', price: 90, countryId: 1001, flag: '🇯🇵' },
    { name: 'Chad', code: '+235', price: 40, countryId: 42, flag: '🇹🇩' },
    { name: 'Bangladesh', code: '+880', price: 170, countryId: 60, flag: '🇧🇩' },
    { name: 'pakistan', code: '+92', price: 40, countryId: 66, flag: '🇵🇰' },
    { name: 'Tanzania', code: '+255', price: 35, countryId: 9, flag: '🇹🇿' },
    { name: 'Mozambique', code: '+258', price: 20, countryId: 80, flag: '��' },
    { name: 'Ukrain', code: '+380', price: 30, countryId: 1, flag: '🇺🇦' },
    { name: 'Morocco', code: '+212', price: 18, countryId: 37, flag: '🇲🇦' },
    { name: 'Kuwait', code: '+965', price: 170, countryId: 100, flag: '🇰🇼' },
    { name: 'canada', code: '+1', price: 17, countryId: 36, flag: '🇨🇦' },
    { name: 'Estonia', code: '+372', price: 44, countryId: 34, flag: '🇪🇪' },
    { name: 'Malaysia', code: '+60', price: 44, countryId: 7, flag: '🇲🇾' },
    { name: 'Angola', code: '+244', price: 39, countryId: 76, flag: '🇦🇴' },
    { name: 'Australia', code: '+61', price: 47, countryId: 175, flag: '🇦🇺' },
    { name: "Cote d'Ivoire (Ivory Coast)", code: '+225', price: 45, countryId: 27, flag: '�🇮' },
    { name: 'Ghana', code: '+233', price: 43, countryId: 38, flag: '��' }
];
...
app.post('/api/auto-payment', async (req, res) => {
    const incomingSecret = String(req.body.secret || '');
    if (!incomingSecret || incomingSecret !== AUTO_PAYMENT_SECRET) {
        console.warn(`Auto-payment rejected due to invalid secret from ${req.ip}`);
        return res.status(401).json({ success: false, message: 'Invalid secret' });
    }

    const senderName = normalizeSenderName(req.body.name);
    ...
    try {
        ...
        if (!paymentRequest) {
            await client.query('COMMIT');
            console.warn(`Auto-payment mismatch: no pending request for sender=${senderName}, amount=${amount}`);
        ...
        const user = userRes.rows[0];
        if (!user) throw new Error('User not found for matched payment request');
        ...
        await client.query(
            ...
            [
                ...
                'deposit',
                'approved',
                `Auto-approved payment request #${paymentRequest.id} via SMS matcher`,
                paymentRequest.transaction_id,
                null
            ]
        );
        await client.query(
            'UPDATE payment_requests SET status = $1, screenshot = NULL, matched_at = CURRENT_TIMESTAMP WHERE id = $2',
            ['approved', paymentRequest.id]
        );
        ...
        return res.json({
            ...
            sender_name: paymentRequest.sender_name,
            amount: Number(paymentRequest.amount || 0)
        });
    } catch (err) {
        await client.query('ROLLBACK');
        console.error('Auto-payment processing failed:', err);
        return res.status(500).json({ success: false, message: 'Auto payment processing failed' });
    } finally {
        client.release();
    }
});
...
app.post('/api/request-payment', ensureAuth, upload.single('screenshot'), async (req, res) => {
    try {
        ...
        if (!senderName) return res.status(400).send('Sender name is required');
